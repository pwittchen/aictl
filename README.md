...
| `view_map` | Display a map (OpenStreetMap base or Esri satellite imagery) with one or more pins. Each input line is one pin: `<query>[ \| <label>[ \| <description>]]` where `<query>` is `<lat>,<lon>`, `<lat>,<lon>,<zoom>`, or a free-form place name geocoded via Nominatim. Multi-pin viewports auto-fit; max 25 pins per call. **Renders only inside the aictl desktop app** — invoking from the CLI/terminal returns an error. Use for "show me where X is", route previews, multi-pin tours, or any geographic comparison; for purely textual location facts prefer `fetch_geolocation` or `search_web_*` |
| `draw_chart` | Render a chart from structured data via Chart.js. Input is a single JSON object: `{"type":"line\|bar\|pie\|doughnut\|scatter","title":"...","x_label":"...","y_label":"...","labels":["A","B"],"series":[{"name":"Series 1","data":[1,2]}]}`. For `line`/`bar` each series's `data` length must match `labels`; `pie`/`doughnut` accept one series and reject negative values; `scatter` ignores `labels` and takes `data` as `[x,y]` pairs (or `{"x":1,"y":2}` objects). Caps: 8 series, 500 points per series, 500 labels. **Renders only inside the aictl desktop app** — invoking from the CLI/terminal returns an error. Chart text, gridlines, and tooltip colours follow the current app theme and re-theme on the fly when the user flips theme (or when the OS appearance changes under `system` mode). Use for trends over time, category comparisons, distributions, or proportions; for raw numbers prefer a Markdown table |
