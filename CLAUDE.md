...
- `src/commands.rs` — REPL slash command handling (`/behavior`, `/clear`, `/compact`, `/context`, `/copy`, `/exit`, `/help`, `/info`, `/model`, `/security`, `/tools`, `/update`). Returns a `CommandResult` enum consumed by the REPL loop in `main.rs`.
