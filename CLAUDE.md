...
- `src/commands.rs` — REPL slash command handling (`/agent`, `/behavior`, `/clear`, `/compact`, `/context`, `/copy`, `/exit`, `/help`, `/info`, `/issues`, `/memory`, `/model`, `/security`, `/session`, `/tools`, `/update`). `MemoryMode` enum (LongTerm/ShortTerm) for conversation history optimization. Returns a `CommandResult` enum consumed by the REPL loop in `main.rs`. Also hosts the `/agent` interactive menu (create manually, create with AI, view all, unload), the `/session` interactive menu (current info, set name, view saved, clear all), the non-interactive `print_sessions_cli` and `print_agents_cli` helpers, and the `--config` interactive configuration wizard (`run_config_wizard`).
