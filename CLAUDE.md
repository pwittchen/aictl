...
**Agents**: Reusable system prompt extensions stored as plain text files in `~/.aictl/agents/<name>`. Managed via `/agent` menu: create manually (multi-line text input), create with AI (LLM generates prompt from description), view all (browse/load/delete), unload. Can also be loaded via `--agent <name>` / `-A <name>` CLI flag (works in both single-shot and interactive modes). When loaded, the agent prompt is appended to the system prompt under `# Agent: <name>`. The REPL prompt shows the agent name in magenta brackets (e.g. `[my-agent] ❯`).
