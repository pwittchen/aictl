...
Single-binary async Rust CLI with nine modules:
...
- `src/llm_openai.rs`, `src/llm_anthropic.rs`, `src/llm_gemini.rs` — provider-specific API call implementations
...
**Providers**: OpenAI (`call_openai`), Anthropic (`call_anthropic`), and Gemini (`call_gemini`) each convert `&[Message]` to provider-specific formats. Anthropic uses a top-level `system` field; OpenAI includes system messages inline; Gemini uses a `systemInstruction` field and maps assistant role to `model`. All return `TokenUsage` for cost tracking and timing display.
