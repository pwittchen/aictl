...
**Flow**: CLI args (clap) → `security::init()` → resolve incognito flag (`--incognito` or `AICTL_INCOGNITO`) → `build_system_prompt()` (base system prompt + optional `AICTL.md` content) → `run_agent_turn` loop → provider call → parse response for `<tool>` tags → security validation → execute tool or print final answer. In interactive mode, the REPL also initializes a session (new uuid or loaded from `--session <id|name>`) before printing the welcome banner.
