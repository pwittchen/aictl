...
- `commands.rs` + `commands/` — slash commands (`/agent`, `/balance`, `/behavior`, `/clear`, `/compact`, `/config`, `/context`, `/copy`, `/exit`, `/gguf`, `/help`, `/history`, `/hooks`, `/info`, `/keys`, `/memory`, `/mlx`, `/model`, `/ping`, `/plugins`, `/retry`, `/roadmap`, `/security`, `/session`, `/skills`, `/stats`, `/tools`, `/undo`, `/uninstall`, `/update`, `/version`); any other `/<name>` falls through to a user-defined skill lookup.
