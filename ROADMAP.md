...
- **Hooks** — User-defined shell commands that the harness executes in response to lifecycle events, modeled after Claude Code's hooks. Configured in `~/.aictl/config` (or a dedicated `~/.aictl/hooks.json`) as a map of event → list of `{ matcher, command, timeout }` entries. Supported events: `SessionStart`, `SessionEnd`, `UserPromptSubmit` (can block or rewrite the prompt), `PreToolUse` (can deny a tool call), `PostToolUse` (observe results), `Stop` (after the agent finishes a turn), `PreCompact`, `Notification`. The matcher is a glob over tool name (`exec_shell`, `write_file`, `mcp__*__*`) or event-specific selector. Each hook receives a JSON payload on stdin (event name, session id, CWD, tool name + args for tool events, prompt text for prompt events) and can return JSON on stdout to influence behavior — `{ "decision": "block", "reason": "..." }` aborts the action and surfaces the reason to the LLM, `{ "additionalContext": "..." }` injects extra context into the next turn. Hooks run with a default 60s timeout, inherit the session's CWD, and are subject to `security::validate_tool`-style path checks; `--unrestricted` does NOT bypass hook execution but does relax the inner shell-validation. Add `/hooks` slash command to list, enable/disable, or test-fire individual hooks, and `--list-hooks` CLI flag. Importantly, hooks are *harness* behavior, not LLM behavior — automated rules like "always run `cargo fmt` after edit_file" belong here, not in agent prompts or memory.
- **Duplicate-call guard: block only consecutive repeats** — Today `tools::is_duplicate_call` consults a session-wide `HashSet` (`CALL_HISTORY` in `src/tools.rs`), so any `(tool_name, normalized_input)` pair that ran once in the session is rejected forever — even when the agent legitimately needs to re-fetch state after intervening work (e.g. `read_file` → `edit_file` → `read_file` to verify the edit, or polling a status endpoint between unrelated steps). Relax this so only **consecutive** identical calls are blocked: an identical call is allowed once a *different* tool call (or a non-tool assistant turn / new user message) has appeared between the two invocations. Replace the `HashSet` with a "last call" slot — `OnceLock<Mutex<Option<(String, String)>>>` — set on every successful dispatch and cleared whenever any other tool runs, the conversation advances past a tool-only turn, or `clear_call_history` fires (REPL `/clear`, session switch). Update `is_duplicate_call` to compare against the slot rather than set membership, and adjust the rejection message in `src/run.rs` (currently "...already called the tool ... earlier in the session...") to say "back-to-back" instead of "earlier in the session". Keep input normalization (`normalize_input`) unchanged. The looping-small-model failure mode that motivated the original guard is specifically *consecutive* repetition, so this preserves the protection while unblocking legitimate re-reads.
