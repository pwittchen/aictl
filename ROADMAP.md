...
- **Provider credit / quota balance** — Show how much credit, tokens, or money is left for each configured provider so the user can see the remaining headroom before hitting a limit. Surface it in `/stats` (or a dedicated `/balance` / `/credits` command) and on the CLI as `--balance` / `--list-balances`. Per provider, fetch via the official billing/usage endpoint when one exists, fall back to whatever rate-limit headers the chat API returns (e.g. `x-ratelimit-remaining-*`) and otherwise report "unknown" rather than guessing. Note: it is not yet confirmed which providers expose this — needs verification per provider (OpenAI, Anthropic, Gemini, Grok, Mistral, DeepSeek, Kimi, Z.ai); local providers (Ollama, GGUF, MLX) are out of scope.
