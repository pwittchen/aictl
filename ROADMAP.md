...
- **Skill templates** — Ship a curated set of built-in skills (review, summarize, inspect-cert, audit-deps, scan-wifi, …) via the remote catalogue so `/skills` → Browse has real content on day one. Action-shaped counterpart to agent templates — single-turn procedures rather than session-long personas. See [.claude/plans/skill-templates.md](.claude/plans/skill-templates.md) for the development plan.
...
- **Sensitive data redaction** — Detect and block or redact sensitive data in user prompts, file reads, tool output, and conversation history before it is sent to a remote LLM provider. Covers personal data (names, surnames, national ID / document numbers, phone numbers, email addresses, street addresses, dates of birth, IBAN / credit card numbers) and credentials (API keys, tokens, passwords, private keys, connection strings, AWS / GCP / Azure access keys, JWTs). Implementation: a pluggable `Redactor` layer in `src/security.rs` invoked on every outbound message in `run_agent_turn` and on every tool result before it rejoins the history. Detection combines high-precision regex packs (key formats, card numbers with Luhn check, IBAN mod-97, common secret shapes) with entropy heuristics for opaque tokens and an optional local NER pass (e.g. a small on-device model via the `mlx` or `gguf` feature) for named entities. Consider using [gline-rs](https://github.com/fbilhaut/gline-rs) for lightweight embedded NER. Three modes via `AICTL_SECURITY_REDACTION=off|redact|block` — `redact` replaces matches with typed placeholders (`[REDACTED:API_KEY]`, `[REDACTED:PHONE]`) deterministically so the LLM still reasons about structure, `block` aborts the turn with a clear explanation of what was detected and where. Local providers (Ollama, GGUF, MLX) bypass redaction by default since data never leaves the machine; toggleable via `AICTL_SECURITY_REDACTION_LOCAL`. User-defined patterns and allowlists live in `~/.aictl/config` (`AICTL_REDACTION_EXTRA_PATTERNS`, `AICTL_REDACTION_ALLOW`). Redaction events are recorded in the per-session audit log alongside tool calls. See [.claude/plans/sensitive-data-redaction.md](.claude/plans/sensitive-data-redaction.md) for the development plan.
