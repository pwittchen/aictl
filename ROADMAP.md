...
- **Decompose large dispatcher functions** — Three top-level dispatchers carry most of the imperative branching in the codebase: `handle_repl_input` (`src/repl.rs:295`, ~332 lines), `main` (`src/main.rs:23`, ~304 lines), and `run_config_wizard` (`src/commands/config_wizard.rs`, ~268 lines). Extract each match arm / phase into a focused helper (e.g. `handle_slash_command`, `handle_user_turn`, `wizard_step_provider`, `init_session_from_args`) so each unit becomes individually testable, the call graph is grep-able, and the diff for any one branch stops touching the giant function. Keep behavior identical — pure refactor, no semantics change. Worth doing in three separate PRs to keep review tractable.
- **Unified `AictlError` enum** — Replace the ~50 `Box<dyn std::error::Error>` return sites across `src/llm/*.rs` and `src/tools/*.rs` with a single `AictlError` enum defined via `thiserror`, with structured variants like `Timeout`, `Auth`, `Provider { provider, status, body }`, `Parse`, `Io`, `Security`, `ToolMissing`. Lets callers branch on the variant (retry on `Timeout`, surface `Auth` to the user with a re-key prompt, fail fast on `Security`) instead of stringly-typed substring matches on error messages. Migrate provider by provider so the change stays incremental; keep `From` impls for `reqwest::Error`, `serde_json::Error`, and `std::io::Error` to avoid touching every `?`. The agent loop in `run::run_agent_turn` becomes the main beneficiary — it can implement smarter retry/timeout policies once errors carry structure.
- **End-to-end CLI smoke tests under `tests/`** — Add an out-of-crate `tests/` directory with end-to-end smoke tests that drive the real binary (via `assert_cmd` or a spawned `cargo run`) using `Provider::Mock` so no network is needed. Cover the two largest currently-untested files — `src/main.rs` and `src/repl.rs` — by exercising: single-shot mode (`aictl --mock "hello"`), REPL slash commands (`/help`, `/model`, `/clear`, `/exit`), agent-loop tool dispatch with a mock tool call in the response, session persistence across two invocations, and the `--unrestricted` / `--quiet` flag plumbing. These catch CLI-surface regressions (arg parsing, exit codes, stdout/stderr framing, REPL dispatch) that the existing in-crate unit tests structurally can't reach.
