...
/// after [`crate::config::load_config`].
pub fn init() {
    let pol = load_policy();
    POLICY.set(pol).ok();
}
...
fn load_policy() -> RedactionPolicy {
    ...
    let extra_patterns =
        parse_extra_patterns(&config_get("AICTL_REDACTION_EXTRA_PATTERNS").unwrap_or_default());
    ...
    .filter_map(|p| match Regex::new(&p) {
        Ok(r) => Some(r),
        Err(e) => {
            eprintln!("Warning: invalid AICTL_REDACTION_ALLOW pattern '{p}': {e}. Skipped.");
            None
        }
    })
    .collect();
    ...
    if ner_requested {
        match ner::status(true) {
            ner::NerStatus::FeatureMissing => eprintln!(
                "Warning: AICTL_REDACTION_NER=true but this build lacks the `redaction-ner` feature. \
                 The NER pass will not run. Rebuild with `cargo build --features redaction-ner`."
            ),
            ner::NerStatus::ModelMissing { expected_name } => eprintln!(
                "Warning: AICTL_REDACTION_NER=true but NER model '{expected_name}' is not on disk. \
...
/// Parse `NAME=REGEX;NAME=REGEX;…` into compiled pairs. Bad regexes
/// surface a warning and are skipped rather than aborting startup.
fn parse_extra_patterns(s: &str) -> Vec<(String, Regex)> {
    let mut out = Vec::new();
    for entry in s.split(';') {
        ...
        let Some((name, pattern)) = entry.split_once('=') else {
            eprintln!(
                "Warning: AICTL_REDACTION_EXTRA_PATTERNS entry '{entry}' is missing '=' — expected NAME=REGEX. Skipped."
            );
            continue;
        };
        let name = name.trim();
        ...
        if name.is_empty() || pattern.is_empty() {
            eprintln!(
                "Warning: AICTL_REDACTION_EXTRA_PATTERNS entry '{entry}' has empty name or pattern. Skipped."
            );
            continue;
        }
        match Regex::new(pattern) {
            ...
            Err(e) => eprintln!(
                "Warning: invalid AICTL_REDACTION_EXTRA_PATTERNS regex for '{name}': {e}. Skipped."
            ),
        }
    }
    out
...
mod tests {
    ...
    fn parse_extra_patterns_basic() {
        let got = parse_extra_patterns("CUSTOMER=CUST-\\d+;TICKET=JIRA-\\d+");
        assert_eq!(got.len(), 2);
    ...
    fn parse_extra_patterns_skips_bad_regex() {
        let got = parse_extra_patterns("GOOD=abc;BAD=(unclosed");
        assert_eq!(got.len(), 1);
    ...
    fn parse_extra_patterns_skips_malformed_entry() {
        let got = parse_extra_patterns("noEquals;HAS=ok");
        assert_eq!(got.len(), 1);
