...
pub async fn call_mlx(
    ...
    // Management is done; inference wiring (mlx-rs + safetensors loader +
    // tokenizer + chat template) is the next step. Until then, surface a
    // clear error instead of silently returning empty output.
    let _ = model_path(model);
    Err("MLX inference is not yet implemented. Model management (pull / list / remove) works; the inference path will land in a follow-up.".into())
}
