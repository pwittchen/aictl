...
- read_document: Read a PDF or DOCX document and extract its content as markdown text. Pass the file path as input. Supports .pdf and .docx files. PDF text is extracted directly; DOCX is converted to markdown preserving headings, lists, tables, and formatting. Use this tool when the user asks to read, analyze, or summarize a PDF or DOCX document.
