    ...
    >
      <div class="agent-editor-panel">
        ...
        <div class="agent-editor-body">
          <Show when={error()}>
            <p class="agent-editor-error">{error()}</p>
          </Show>
          <Show when={info()}>
            <p class="agent-editor-info">{info()}</p>
          </Show>
          <div class="agent-editor-row">
            <label for="agent-editor-name">Name</label>
            <input
          ...
          <Show when={mode() === "ai"}>
            <div class="agent-editor-row">
              <label for="agent-editor-desc">Description</label>
              <input
                id="agent-editor-desc"
                type="text"
                placeholder="what should this agent do?"
                value={description()}
                onInput={(e) => setDescription(e.currentTarget.value)}
              />
              <p class="agent-editor-help">
                Generates the prompt body using the active provider/model
                (Settings → Model). Review the result before saving.
              </p>
              <div class="agent-editor-actions inline">
                <button
                  type="button"
          ...
          <div class="agent-editor-row">
            <label for="agent-editor-body">Prompt</label>
            <textarea
              id="agent-editor-body"
              rows={mode() === "ai" ? 12 : 16}
              placeholder={
                mode() === "ai"
