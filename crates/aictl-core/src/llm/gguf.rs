...
/// Download a model to `~/.aictl/models/gguf/<name>.gguf`. Reports progress
/// via the supplied UI (interactive frontends render a bar; single-shot /
/// non-terminal frontends silently no-op). Overwrites any existing file
/// with the same name. Returns the resolved local name.
pub async fn download_model(
    ...
    let progress = ui.progress_begin(&name, (total > 0).then_some(total));
