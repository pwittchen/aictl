...
/// `~/.aictl/models/mlx/<name>/`. Reports per-file progress via the
/// supplied UI; overwrites any existing directory with the same name.
...
pub async fn download_model(
    ...
    for (idx, file) in files.iter().enumerate() {
        ...
        let label = format!("[{}/{}] {file}", idx + 1, files.len());
