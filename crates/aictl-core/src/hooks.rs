...
impl HookEvent {
    ...
    // Inherent rather than `FromStr` because the success case returns
    // `Option<Self>` (not `Result`) — case-insensitive match against the
    // event names; `None` means "unknown event".
...
#[allow(clippy::implicit_hasher)] // callers always use the default HashMap hasher
