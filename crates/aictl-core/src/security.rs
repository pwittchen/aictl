...
/// Returns warnings produced while compiling the redaction policy (bad
/// regex patterns, missing NER feature/model) so the caller can surface
/// them through the active UI rather than this module reaching for
/// `eprintln!` directly.
#[must_use]
pub fn init(unrestricted: bool) -> Vec<String> {
