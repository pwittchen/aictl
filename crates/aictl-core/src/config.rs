...
- generate_image: Generate an image from a text description. Pass the image description as input. The generated image is saved as a PNG to the current directory and the file path is returned. Supports three providers (auto-selected based on available keys, active provider preferred): GPT Image (LLM_OPENAI_API_KEY), Imagen (LLM_GEMINI_API_KEY), or Grok (LLM_GROK_API_KEY). Use this tool when the user asks you to create, generate, draw, or make an image or picture.
...
- draw_chart: Render a chart (line, bar, pie, doughnut, or scatter) from structured data. Input is a single JSON object: `{"type":"line|bar|pie|doughnut|scatter","title":"optional title","x_label":"optional","y_label":"optional","labels":["A","B",...],"series":[{"name":"Series 1","data":[1,2,3]}, ...]}`. For `line`/`bar` each series's `data` length must match `labels`; `pie`/`doughnut` accept exactly one series and reject negative values; `scatter` ignores `labels` and takes `data` as `[x,y]` pairs (e.g. `[[1,2],[3,4]]`) — `{"x":1,"y":2}` objects also work. Up to 8 series, 500 points per series, 500 labels. Examples: bar comparing two years — `{"type":"bar","title":"Sales","labels":["Q1","Q2","Q3","Q4"],"series":[{"name":"2024","data":[120,140,150,170]},{"name":"2025","data":[150,170,180,210]}]}`; pie share — `{"type":"pie","title":"Market share","labels":["A","B","C"],"series":[{"name":"share","data":[40,30,30]}]}`; scatter — `{"type":"scatter","title":"Weight vs Height","series":[{"name":"sample","data":[[170,65],[180,80],[160,55]]}]}`. **This tool only renders inside the aictl desktop app — when invoked from the CLI/terminal it returns an error and must not be retried.** Chart colours and gridlines adapt to the user's current light/dark theme automatically. Use it when the user asks to see, plot, graph, chart, or visualise data, or when a chart obviously helps the answer (trends over time, category comparisons, distributions, proportions); for raw numeric output prefer a Markdown table. Output includes a `[draw_chart] {json}` marker the desktop intercepts to render the chart; the model should also relay a brief textual takeaway so users in non-desktop frontends still get the substance.
- view_map: Display a map (OpenStreetMap base or Esri satellite imagery, switchable in-app) with one or more pins. Each non-empty input line is one pin in the form `<query>[ | <label>[ | <description>]]`, where `<query>` is `<lat>,<lon>` (e.g. `48.8566, 2.3522`), `<lat>,<lon>,<zoom>` (zoom 1..=19, default 13; only honored for single-pin input), or a free-form place name geocoded via Nominatim. Optional `<label>` overrides the auto-derived name; optional `<description>` is shown in a popup when the user clicks the pin. Examples: single pin — `Eiffel Tower, Paris` or `48.86, 2.35, 15`; multi-pin tour — three lines, e.g. `Eiffel Tower | Eiffel Tower | 324m wrought-iron tower`, `Louvre Museum | Louvre | World's largest art museum`, `Notre-Dame, Paris | Notre-Dame | Medieval cathedral, reopened 2024`. With multiple pins the viewport auto-fits to enclose all of them; max 25 pins per call. **This tool only renders inside the aictl desktop app — when invoked from the CLI/terminal it returns an error and must not be retried.** Use it when the user asks to see, show, plot, compare, or pin locations, or when a map clearly helps the answer (city tour, route preview, geographic comparison, "where is …"); for purely textual location facts (capital of a country, distance, timezone) prefer `fetch_geolocation` or `search_web_*` instead. Output includes a `[view_map] {json}` marker the desktop intercepts to render the map; the model should also relay the human-readable labels so users in non-desktop frontends still get the locations in text.
...
Tool-call syntax (strict — deviations break the call):
- The text between `<tool name="…">` and `</tool>` is the RAW input the tool description asks for. Never box it in `<input>`, `<parameter>`, `<arg>`, JSON, or any other markup; never repeat the opening `<tool …>` tag inside the block.
- Never wrap the block in a container such as `<function_calls>`, `<tool_calls>`, or `<invoke>`. Close every block with `</tool>` exactly — no other closing tag.
- Never write `<tool_result>` or `<tool_results>` blocks yourself, and never invent a tool's output. The host runs the tool and sends the real result back to you in the next turn.
- Stop generating as soon as you have emitted your tool block(s). Do not describe what the tool produced, and do not summarize the finished work, until you have seen the actual result.
...
/// `HOME` is unset so the calling binary can decide how to surface the
/// error (the CLI prints it and exits; future frontends — server, embedded
/// engine — may want to handle it differently). When the file is missing
/// or unreadable an empty cache is installed and `Ok(())` is returned.
///
/// # Errors
///
/// Returns `Err("HOME environment variable not set")` if `HOME` is not
/// in the process environment.
pub fn load_config() -> Result<(), &'static str> {
