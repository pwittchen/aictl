...
//! `serde_json::Error`, `std::io::Error`) plus [`crate::run::Interrupted`]
//! and bare `String` / `&str` for ergonomic `into()` conversion at the
//! edges. REPL-only error sources (rustyline) live on the CLI side and
//! are mapped manually at the call site.
