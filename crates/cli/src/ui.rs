...
//!   no spinner, no streaming framing beyond a `\r\n` translation that
//!   lets streamed answers coexist with the raw-mode Esc listener.
//! * [`InteractiveUI`] — REPL output: termimad-rendered markdown, a
//!   `crossterm` selector for tool confirmation, `indicatif` progress
//!   bars for downloads, and the raw-mode Esc cancel listener.
...
// ── Progress backend (indicatif glue) ───────────────────────────────

/// `ProgressBackend` wrapper around an [`indicatif::ProgressBar`]. Held
/// behind a `Mutex` because the engine borrows the UI immutably while
/// the bar mutates internal state on every tick.
struct IndicatifBackend(Mutex<ProgressBar>);

impl ProgressBackend for IndicatifBackend {
    fn update(&self, current: u64, message: Option<&str>) {
        let Ok(pb) = self.0.lock() else { return };
    ...
    fn finish(&self, final_message: Option<&str>) {
        ...
        if let Some(m) = final_message {
            pb.finish_with_message(m.to_string());
        } else {
            pb.finish_and_clear();
        }
    }
...
impl AgentUI for PlainUI {
    ...
    fn confirm_tool(&self, tool_call: &ToolCall) -> ToolApproval {
        ...
        match input.trim() {
            "y" | "Y" | "yes" | "Yes" => ToolApproval::Allow,
            _ => ToolApproval::Deny,
        }
    }
...
impl InteractiveUI {
    ...
    /// Horizontal rule drawn above status / summary lines.
    fn status_rule() {
        let dashes = "─".repeat(rule_width());
    ...
    /// and clears `stream_word` / `stream_spaces`.
...
impl AgentUI for InteractiveUI {
    ...
    fn warn(&self, text: &str) {
        eprintln!("{PAD}{}", format!("Warning: {text}").with(Color::Yellow));
    }
    ...
    fn interruption(&self) -> Pin<Box<dyn Future<Output = ()> + Send + '_>> {
        Box::pin(async move {
            ...
            // Bracket the polling loop with a Drop guard so raw mode is
            // disabled whether the future completes (Esc pressed) or is
            // dropped because the surrounding `select!` resolved on the
            // other branch (LLM call returned first).
            struct RawModeGuard;
            impl RawModeGuard {
                fn new() -> Self {
                ...
                let pressed = tokio::task::spawn_blocking(|| {
                    if event::poll(Duration::from_millis(50)).unwrap_or(false) {
                        matches!(
                            event::read(),
                            Ok(Event::Key(k))
                                if k.code == KeyCode::Esc && k.kind == KeyEventKind::Press
                        )
                    } else {
                        false
                    }
