...
A separate path lives in `crates/aictl-core/src/llm/server_proxy.rs`. When `config::active_server()` returns `Some((url, key))` (i.e. `AICTL_CLIENT_HOST` is set and `AICTL_CLIENT_MASTER_KEY` resolves) and the resolved provider is non-local, `run::run_agent_turn` dispatches through `server_proxy::call` instead of the per-provider module above. The proxy reuses `llm/openai`'s `pub(crate)` request/response types verbatim, posts to `${AICTL_CLIENT_HOST}/v1/chat/completions` with `Authorization: Bearer ${AICTL_CLIENT_MASTER_KEY}`, and streams via the same `drive_openai_compatible_stream` helper. Local providers (Ollama / GGUF / MLX) bypass the server unconditionally — `Provider::is_local()` is the gate. The model catalogue, pricing, security gate, redaction, audit log, and session writer all stay CLI-side; the server is purely the LLM transport. A once-per-process `GET /healthz` probe runs before the first proxied request and surfaces its result through `ui::warn_global`. `/balance` reads the server's `/v1/stats` aggregate when routing is active.
