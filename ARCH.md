...
crates/aictl-cli/src/
 ...
 ├── commands/          One submodule per slash command (agent, balance, behavior, clipboard, compact, config_wizard, gguf, help, history, hooks, info, keys, mcp, memory, menu, mlx, model, ping, plugins, roadmap, security, serve, session, skills, stats, tools, uninstall, update). `/retry` and `/undo` reuse aictl_core::transcript::{retry_last_exchange, undo_turns} re-exports rather than carrying their own submodules. MemoryMode is re-exported from aictl_core::run.
