...
crates/aictl-cli/src/
 ...
 ├── commands.rs        REPL slash-command dispatch + CommandResult enum (/agent, /balance, /behavior, /clear, /compact, /config, /context, /copy, /exit, /gguf, /help, /history, /hooks, /info, /keys, /mcp, /memory, /mlx, /model, /ping, /plugins, /remember, /retry, /roadmap, /security, /session, /skills, /stats, /tools, /undo, /uninstall, /update, /version); unrecognized /<name> falls through to skills::find for user-authored skill invocation
 ├── commands/          One submodule per slash command (agent, balance, behavior, clipboard, compact, config_wizard, gguf, help, history, hooks, info, keys, mcp, menu, mlx, model, ping, plugins, roadmap, security, serve, session, skills, stats, tools, uninstall, update). `/retry` and `/undo` reuse aictl_core::transcript::{retry_last_exchange, undo_turns} re-exports rather than carrying their own submodules.
...
crates/aictl-core/src/
 ...
 ├── keys.rs            Secure API key storage. System keyring (Keychain / Secret Service) with transparent plain-text fallback. lock_key/unlock_key/clear_key migration primitives. On macOS, the `keys/macos_keychain.rs` submodule wraps SecItemAdd/CopyMatching/Delete via security-framework-sys to store items under a shared `<TeamID>.com.piotrwittchen.aictl` access group declared in each binary's entitlements.plist — the signed CLI / desktop / server share one Keychain partition without per-binary password prompts. Cached at first access via `shared_keychain_active()`; legacy unscoped items are migrated across on first read; ad-hoc-signed dev builds fall back to the unscoped keyring::Entry path.
