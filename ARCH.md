...
crates/aictl-cli/src/
 ├── main.rs            CLI args (clap) and management-flag dispatch (--version, --update, --uninstall, --config, agent/skill/session/key/model helpers, --audit-file, --cwd), provider/model/key resolution, then routes to single-shot or REPL. apply_cwd_override anchors std::env::set_current_dir before any subsystem reads the launch dir, so the security jail root, prompt-file lookup, and project-local agent/skill resolution all flip from one place. Re-exports aictl-core modules (config, llm, run, security, …) under crate::* so legacy paths in REPL / slash-command code keep resolving.
 ...
 ├── commands.rs        REPL slash-command dispatch + CommandResult enum (/agent, /balance, /behavior, /clear, /coding, /compact, /config, /context, /copy, /exit, /gguf, /help, /history, /hooks, /info, /keys, /mcp, /memory, /mlx, /model, /ping, /plugins, /remember, /retry, /roadmap, /security, /session, /skills, /skip, /stats, /tools, /undo, /uninstall, /update, /version); unrecognized /<name> falls through to skills::find for user-authored skill invocation. `/coding` keeps `/coding-agent` / `/coding_agent` as silent legacy aliases. `/skip` takes an optional `review` / `test` argument to skip those coding-agent phases
 ├── commands/          One submodule per slash command (agent, balance, behavior, clipboard, coding_agent, compact, config_wizard, gguf, help, history, hooks, info, keys, mcp, menu, mlx, model, ping, plugins, roadmap, security, serve, session, skills, stats, tools, uninstall, update). `/retry`, `/undo`, and `/skip` reuse aictl_core::transcript::{retry_last_exchange, undo_turns} re-exports / inline REPL handlers rather than carrying their own submodules.
 ├── ui.rs              PlainUI (single-shot, pipe-friendly; OutputFormat selects md (default) / text / json — text strips markdown, json emits an {"answer","model","provider"} envelope) + InteractiveUI (REPL: termimad markdown, crossterm tool-confirm selector, indicatif progress backend, raw-mode Esc cancel listener). Re-exports aictl_core::ui::{AgentUI, ProgressHandle, ProgressBackend, ToolApproval} so legacy crate::ui::AgentUI paths keep resolving.
...
crates/aictl-core/src/
 ...
 ├── coding.rs          [experimental] Coding-agent mode helpers — WorkflowPhase enum + `<phase>NAME</phase>` tag parser, per-turn phase_hint prose, project linter / test-command auto-detection (cargo / npm / pytest / go). Gated on AICTL_CODING_AGENT (default false); `coding_agent_enabled()` short-circuits to false under Role::Server so the HTTP proxy never adopts the coding prompt
 ├── config.rs          Config file loading (~/.aictl/config) into RwLock-backed cache, constants (system prompt, coding-agent prompt SYSTEM_PROMPT_CODING, spinner phrases, agent loop limits), project prompt file loading; load_config returns Result so the CLI can surface a HOME-missing error
 ...
 ├── tools.rs           XML tool-call parsing, tool execution dispatch (security gate + output sanitization), duplicate-call guard, TOOL_COUNT (35)
 ├── tools/             One submodule per tool (archive, calculate, check_port, checksum, clipboard, csv_query, datetime, diff, document, draw_chart, filesystem, geo, git, image, json_query, lint, list_processes, memory, notify, run_code, shell, system_info, util, view_map, web)
...
```
 ...
 │                              agents/skills). Bad path = loud exit, no    │
 │                              silent fallback.                            │
...
### Routing through `aictl-server`

A separate path lives in `crates/aictl-core/src/llm/server_proxy.rs`. The proxy is reached **only** when `Provider::AictlServer` is the active provider (selected via `--provider aictl-server`, `AICTL_PROVIDER=aictl-server`, or the `aictl-server:` section in `/model`); picking any other provider goes straight to that provider's API regardless of whether `AICTL_CLIENT_HOST` is set, so the user's chosen provider wins. When `Provider::AictlServer` is active, `run::run_agent_turn` dispatches through `server_proxy::call` instead of the per-provider module above: `config::active_server()` resolves `(AICTL_CLIENT_HOST, AICTL_CLIENT_MASTER_KEY)`, the proxy reuses `llm/openai`'s `pub(crate)` request/response types verbatim, posts to `${AICTL_CLIENT_HOST}/v1/chat/completions` with `Authorization: Bearer ${AICTL_CLIENT_MASTER_KEY}`, and streams via the same `drive_openai_compatible_stream` helper. Missing config surfaces a clear `AictlError::Other` from the agent loop. The model catalogue, pricing, security gate, redaction, audit log, and session writer all stay CLI-side; the server is purely the LLM transport. A once-per-process `GET /healthz` probe runs before the first proxied request and surfaces its result through `ui::warn_global`. `/balance` reads the server's `/v1/stats` aggregate when routing is active. CLI flags `--client-url <URL>` and `--client-master-key <KEY>` override the persisted config for one process without writing to disk.
...
```
 ...
 Also: /agent (Agent), /balance (Balance), /behavior (Behavior), /context (Context), /history (History), /hooks (Hooks), /info (Info), /gguf (Gguf), /mcp (Mcp), /memory (Memory), /mlx (Mlx), /ping (Ping), /plugins (Plugins), /remember (Remember), /security (Security), /session (Session), /skills (Skills), /model (Model), /tools (Continue), /stats (Stats), /keys (Keys), /config (Config), /retry (Retry), /roadmap (Roadmap), /undo (Undo), /update (Update), /uninstall (Uninstall), /version (Version). Any other /<name> the dispatcher doesn't recognize is tried as a skills::find lookup; on a hit it returns CommandResult::InvokeSkill, otherwise the "unknown command" error fires.
...
```
 ~/.aictl/
  ...
  ├── memory.json         Long-term memory store ({"memories": [{"id", "text", "created_at"}]}); written by the save_memory tool, /remember, --remember; read by build_system_prompt; gated by AICTL_MEMORY_ENABLED (default true) and incognito mode
...
- **Redaction**: `AICTL_SECURITY_REDACTION` (`off` / `redact` / `block`, default `off`), `AICTL_SECURITY_REDACTION_LOCAL` (default `false` — local providers bypass), `AICTL_REDACTION_DETECTORS` (subset of `api_key, aws, aws_secret, jwt, private_key, connection_string, credit_card, iban, email, phone, url_secret, ssn, pesel, ip_address, mac_address, high_entropy`), `AICTL_REDACTION_EXTRA_PATTERNS` (semicolon-separated `NAME=REGEX` pairs → `[REDACTED:NAME]`), `AICTL_REDACTION_ALLOW` (semicolon-separated allowlist regexes), `AICTL_REDACTION_NER` (enable Layer-C NER, requires the `redaction-ner` cargo feature + a pulled model), `AICTL_REDACTION_NER_MODEL` (default `onnx-community/gliner_small-v2.1`). See `security/redaction.rs` and `security/redaction/ner.rs`.
...
- **aictl-server client routing**: `AICTL_CLIENT_HOST` (base URL of the upstream `aictl-server`, e.g. `http://127.0.0.1:7878`; empty/unset → direct providers, the default), `AICTL_CLIENT_MASTER_KEY` (Bearer token presented to the server; same lock/unlock lifecycle as the provider keys via `/keys`). Set together; the URL alone aborts startup with a clear error. CLI flags `--client-url <URL>` and `--client-master-key <KEY>` override per-process without persisting. Naming is deliberately distinct from the server's own `AICTL_SERVER_MASTER_KEY` so the same `~/.aictl/config` can host both client and server roles unambiguously.
