...
- No official skill catalogue or browse-from-GitHub flow in this plan. That's a follow-on — see `skills-remote-catalogue.md`, which layers on top of this one once the core feature is stable.
...
Frontmatter parsing: a simple hand-rolled YAML subset parser (key-value lines between `---` fences) keeps us off the `serde_yaml` dependency. Only `name` and `description` are recognized in v1; **unknown fields are silently ignored** so future additions (e.g. `source` and `category` introduced by the remote-catalogue plan) don't break older clients.
